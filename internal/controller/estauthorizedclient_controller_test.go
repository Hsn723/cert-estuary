...
func createTestEAC(name, namespace string, autoApprove, autoRenew, removeExpired bool) {
...
var _ = Describe("ESTAuthorizedClient Controller", func() {
	Context("When reconciling a resource", func() {
		...
		It("should not approve non-owned CSRs", func() {
			...
			createTestEAC(eacName, namespace, true, true, true)
		...
		It("should successfully reconcile the resource", func() {
			...
			createTestEAC(eacName, namespace, true, true, false)
			...
			Expect(meta.IsStatusConditionTrue(eac.Status.Conditions, certestuaryv1.ESTAuthorizedClientConditionTypeReady)).To(BeFalse())
			Expect(eac.Status.CurrentCSRName).To(BeEmpty())
			Expect(eac.Spec.RemoveExpired).To(BeFalse())
			...
			csr.SetOwnerReferences([]metav1.OwnerReference{
				{
			...
			By("Making sure certificates are automatically renewed")
			time.Sleep(1 * time.Second)
			currentCSRName := eac.Status.CurrentCSRName
			_, err = reconciler.Reconcile(ctx, reconcileRequest)
			Expect(err).NotTo(HaveOccurred())
			Expect(k8sClient.Get(ctx, eacNamespacedName, eac)).NotTo(HaveOccurred())
			Expect(meta.IsStatusConditionTrue(eac.Status.Conditions, certestuaryv1.ESTAuthorizedClientConditionTypeReady)).To(BeFalse())
			Expect(eac.Status.PreviousCSRName).To(Equal(currentCSRName))
			Expect(eac.Status.CurrentCSRName).NotTo(Equal(currentCSRName))
		})

		It("should cleanup expired CSRs", func() {
			eac := &certestuaryv1.ESTAuthorizedClient{}
			namespace := createTestNamespace(ctx)
			eacName := uuid.NewString()
			csrName := fmt.Sprintf("%s-%d", eacName, time.Now().Unix())
			...
			createTestEAC(eacName, namespace, true, false, true)
			...
			Expect(meta.IsStatusConditionTrue(eac.Status.Conditions, certestuaryv1.ESTAuthorizedClientConditionTypeReady)).To(BeFalse())
			Expect(eac.Status.CurrentCSRName).To(BeEmpty())
			Expect(eac.Spec.RemoveExpired).To(BeTrue())
			Expect(eac.Spec.AutoRenew).To(BeFalse())
			...
			By("Making sure expired certificates are cleaned up")
			_, err = reconciler.Reconcile(ctx, reconcileRequest)
			Expect(err).NotTo(HaveOccurred())
			Expect(k8sClient.Get(ctx, eacNamespacedName, eac)).NotTo(HaveOccurred())
			Expect(meta.IsStatusConditionTrue(eac.Status.Conditions, certestuaryv1.ESTAuthorizedClientConditionTypeReady)).To(BeFalse())
			Expect(eac.Status.CurrentCSRName).NotTo(Equal(csrName))
			csr = &certsv1.CertificateSigningRequest{}
			err = k8sClient.Get(ctx, csrNamespacedName, csr)
			Expect(client.IgnoreNotFound(err)).NotTo(HaveOccurred())
			Expect(csr).To(Equal(&certsv1.CertificateSigningRequest{}))
		})
	})
})
