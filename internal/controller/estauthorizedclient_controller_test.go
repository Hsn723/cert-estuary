...
var _ = Describe("ESTAuthorizedClient Controller", func() {
	Context("When reconciling a resource", func() {
		...
		estauthorizedclient := &certestuaryv1.ESTAuthorizedClient{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind ESTAuthorizedClient")
			err := k8sClient.Get(ctx, typeNamespacedName, estauthorizedclient)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ESTAuthorizedClient")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ESTAuthorizedClientReconciler{
				Client: k8sClient,
